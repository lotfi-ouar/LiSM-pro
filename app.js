...
async function initDatabaseSync() {
    ...
    try {
        const res = await fetch(syncServerUrl + '/api/db', { cache: 'no-store' });
        if (res.ok) {
...
async function sendDataToServer() {
    ...
    try {
        ...
        const res = await fetch(syncServerUrl + '/api/sync', {
            method: 'POST',
...
function startLanSyncPolling() {
    ...
    syncInterval = setInterval(async () => {
        ...
        syncServerUrl = getSyncServerBaseUrl();
        try {
            const res = await fetch(syncServerUrl + '/api/sync-check', { cache: 'no-store' });
            if (res.ok) {
                const data = await res.json();
                if (data.dbVersion && data.dbVersion > currentDbVersion) {
                    currentDbVersion = data.dbVersion;
                    isSyncing = true;
                    const dbRes = await fetch(syncServerUrl + '/api/db', { cache: 'no-store' });
                    if (dbRes.ok) {
...
window.checkAppLicense = function() {
    try {
        ...
        const saved = localStorage.getItem("shop_license_data");
        let licenseData = saved ? JSON.parse(saved) : null;
        
        // افتراضياً بدون إدخال كود، البرنامج مفعّل دائماً مدى الحياة
        if (!licenseData || !licenseData.expiry || licenseData.expiry === "unlimited") {
        ...
        const now = Date.now();
        const expiryTime = Number(licenseData.expiry);
        
        if (isNaN(expiryTime) || now > expiryTime) {
            // منتهي الصلاحية -> إظهار شاشة الحظر
            if (overlay) overlay.style.display = "flex";
            updateLicenseUIStatus("منتهي الصلاحية ⚠️ (يلزم التجديد)");
            return false;
        } else {
            // مفعّل -> حساب المتبقي
            if (overlay) overlay.style.display = "none";
            const daysLeft = Math.ceil((expiryTime - now) / (1000 * 60 * 60 * 24));
            updateLicenseUIStatus(`مفعّل (متبقي ${daysLeft} يوم)`);
            
            // يظهر تحذير أحمر في أقصى أسفل اليسار فقط إذا كان المتبقي أقل من 7 أيام (أسبوع)
            if (daysLeft <= 7) {
...
window.renderSettingsTab = function() {
    ...
    if (statusEl) {
        const savedLicense = localStorage.getItem("shop_license_data");
        if (savedLicense) {
            try {
                ...
                if (data.expiry === "unlimited") {
                    ...
                    const expiryTime = parseInt(data.expiry) || 0;
                    const remainingMs = expiryTime - Date.now();
                    const days = Math.ceil(remainingMs / (1000 * 60 * 60 * 24));
                    if (days > 0) {
                        statusEl.innerText = `🟢 ترخيص نسختك: نشط (متبقي ${days} يوماً)`;
                        statusEl.style.color = "var(--color-success)";
                    } else {
                        statusEl.innerText = `🔴 الترخيص منتهي! يرجى إدخال كود جديد.`;
                        statusEl.style.color = "var(--color-danger)";
                    }
                }
            } catch(e) {
                statusEl.innerText = "⚠️ لم يتم العثور على ترخيص مفعل.";
